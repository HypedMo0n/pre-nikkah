...
export function AnswerForm({
  ...
  return (
    <form action={action} className="mt-8" ref={formRef}>
      ...
      {type === "single" && (
        <fieldset className="grid gap-3">
          <legend className="sr-only">
            {d["question.answerChoices"]}
          </legend>

          {(options ?? []).map((option) => (
            <label
              className={cn(
                "flex min-h-14 cursor-pointer items-center rounded-expressive border p-4 text-sm font-semibold focus-within:ring-2 focus-within:ring-ink",
                value === option.id
      ...
      {type === "scale" && (
        <div>
          <label className="sr-only" htmlFor="scale-answer">
            {d["question.scaleLabel"]}
          </label>
          ...
          <input
            ...
            max="5"
            min="1"
            onChange={(event) => saveImmediately(event.target.value)}
            type="range"
            value={value}
          />
      ...
      {(state.status === "journey_required" ||
        state.status === "question_unavailable") && (
        <Link
      ...
      <nav
        aria-label={d["question.navigation"]}
        className="mt-7 grid grid-cols-1 gap-3 min-[360px]:grid-cols-2"
      >
        {previous ? (
          <Link
        ...
        <Link
          ...
          className={buttonClasses({
            className: !canContinue
              ? "pointer-events-none border-border bg-border text-ink-soft"
              : undefined,
          })}
          href={next ?? complete}
          onClick={(event) => {
