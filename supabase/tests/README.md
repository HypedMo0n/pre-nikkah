...
| `privacy_and_lifecycle.test.sql` | 43 | Un-shared answers and private notes are unreadable by a partner through any route; a share is revocable by its author and does not survive the space closing; `comparisons` is writable only by `refresh_comparison()`, never by a client; progress functions return counts only |
