...
| `topic_partner_visibility.test.sql` | 17 | Partner state is withheld for topics the couple has not reached, through every route including the progress RPC, comparisons, events and the save return value |
| `disclosure_attestations.test.sql` | 36 | Attestations are owner-only including after a reveal; reveal is per attestation and explicitly confirmed; the overview returns counts without category identity; both disclosure functions lock in one order |
