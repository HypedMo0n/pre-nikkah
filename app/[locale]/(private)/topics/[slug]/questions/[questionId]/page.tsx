...
export default async function QuestionPage({ params }: { params: Promise<{ locale: string; slug: string; questionId: string }> }) {
  ...
  if (coupleResult.error || connectionResult.error || connectionStatus !== "active" || !coupleId) {
    const message = connectionStatus === "waiting"
      ? d["answer.waitingJourney"]
      : connectionStatus === "closed"
        ? d["answer.closedJourney"]
        : d["answer.journeyRequired"];
    const primaryHref = connectionStatus === "waiting"
      ? localizedPath(locale, "/onboarding/waiting-journey")
    ...
    return (
      <main className="safe-screen min-h-[100svh] bg-background"><div className="mx-auto flex min-h-[100svh] w-full min-w-0 max-w-xl flex-col px-4 py-5 min-[360px]:px-5 sm:px-7">
        ...
        <section className="flex-1 pt-8 sm:pt-10"><p className="text-xs font-semibold uppercase tracking-wider text-accent">{topic.name}</p><h1 className="font-expressive mt-3 text-3xl font-medium leading-tight text-ink">{question.text}</h1><div className="mt-6 rounded-productive border bg-card p-5"><p className="text-sm leading-6 text-body">{message}</p><div className="mt-5 grid gap-3"><Link className={buttonClasses()} href={primaryHref}>{connectionStatus === "waiting" ? d["answer.resolveWaiting"] : d["answer.returnDashboard"]}</Link>{connectionStatus === "not_connected" || connectionStatus === null ? <><Link className={buttonClasses({ variant: "secondary" })} href={localizedPath(locale, "/journey")}>{d["answer.createJourney"]}</Link><Link className={buttonClasses({ variant: "secondary" })} href={localizedPath(locale, "/join")}>{d["answer.joinInvite"]}</Link></> : null}</div></div></section>
      </div></main>
    );
  }

  const { data: answer } = await supabase.from("answers").select("value,importance,discussion_preference").eq("user_id", user.id).eq("couple_id", coupleId).eq("question_id", questionId).maybeSingle();
