...
export default async function DashboardPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<{ joined?: string }> }) {
  ...
  return (
    <OnboardingShell locale={locale} productive>
      ...
      {connection.status === "active" && (
        <>
          <section className="mt-6 grid grid-cols-2 gap-3" aria-label={d["dashboard.metricsTitle"]}>
      ...
      {connection.status === "active" && (
        <section className="mt-7" aria-labelledby="up-next-heading">
          ...
          <div className="mt-4 space-y-3">
            {activeTopics.length === 0 ? <Card className="p-5"><p className="font-semibold text-ink">{d["dashboard.allCompleteTitle"]}</p><p className="mt-2 text-sm leading-6 text-body">{d["dashboard.allCompleteBody"]}</p></Card> : activeTopics.map((summary) => {
              const types = questions.filter((question) => question.topic_id === summary.topic.id).map((question) => question.type as QuestionType);
              return (
                <Card className="p-4" key={summary.topic.id}>
                  <div className="flex items-start justify-between gap-4"><div><h3 className="font-semibold text-ink">{summary.topic.name}</h3><p className="mt-1 text-sm text-ink-soft">{summary.currentUserCompletedCount}/{summary.totalQuestionCount} {d["dashboard.yourQuestions"]} · {summary.bothCompletedCount}/{summary.totalQuestionCount} {d["dashboard.togetherQuestions"]} · {estimateTopicMinutes(types)} {d["topic.minutes"]}</p></div><span className="rounded-full bg-section px-3 py-1 text-xs font-semibold text-ink-soft">{d[stageLabelKeys[summary.stage]]}</span></div>
                  {summary.isCurrent && summary.unansweredQuestions.length > 0 && <ul className="mt-4 space-y-2 text-sm leading-6 text-body">{summary.unansweredQuestions.map((question) => <li key={question.id}>• {question.text}</li>)}</ul>}
                  {summary.isCurrent && summary.stage === "waiting_for_partner" && <p className="mt-4 text-sm leading-6 text-body">{d["dashboard.waitingPartnerBody"]}</p>}
                  {summary.isCurrent && summary.stage === "ready_to_discuss" && <p className="mt-4 text-sm leading-6 text-body">{d["dashboard.readyDiscussBody"]}</p>}
                  <Link className={buttonClasses({ variant: summary.isCurrent ? "primary" : "secondary", className: "mt-4 w-full" })} href={localizedPath(locale, `/topics/${summary.topic.slug}`)}>{summary.isCurrent ? d["common.continue"] : d["dashboard.previewTopic"]}<ArrowRight aria-hidden="true" size={18} /></Link>
                </Card>
              );
            })}
          </div>
