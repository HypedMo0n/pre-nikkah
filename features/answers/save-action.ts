...
export async function saveAnswerAction(previous: AnswerSaveState, formData: FormData): Promise<AnswerSaveState> {
  ...
  const fail = (): AnswerSaveState => ({ status: "error", message: translate(locale, "status.error"), savedValue: previous.savedValue });
  ...
  if (questionError || coupleError || !question || !coupleId) return fail();
  ...
  if (questionCount && answerCount === questionCount) {
    await supabase.from("topic_progress").upsert({ couple_id: coupleId, topic_id: question.topic_id, user_id: user.id, completed_at: new Date().toISOString() }, { onConflict: "couple_id,topic_id,user_id" });
  }
