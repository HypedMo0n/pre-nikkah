...
describe("Supabase environment compatibility", () => {
  ...
  const publicEnv = readFileSync(
    path.join(process.cwd(), "lib", "env", "public.ts"),
    "utf8",
  );
  ...
  it("maps modern public Supabase variables into the browser-safe aliases", () => {
    ...
    expect(publicEnv).toContain("process.env.SUPABASE_URL");
    expect(publicEnv).toContain("process.env.SUPABASE_PUBLISHABLE_KEY");
